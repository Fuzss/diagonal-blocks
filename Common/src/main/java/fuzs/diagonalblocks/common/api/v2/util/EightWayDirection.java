package fuzs.diagonalblocks.common.api.v2.util;
...
/**
 * @see Direction
 * @see net.minecraft.core.Direction8
 */
public enum EightWayDirection implements StringRepresentable {
    ...
    private static final Map<String, EightWayDirection> DIRECTIONS_BY_KEY = Stream.of(EightWayDirection.values())
            .collect(Collectors.toMap(EightWayDirection::getSerializedName, Function.identity()));
    ...
    public EightWayDirection[] getCardinalNeighbors() {
        if (this.isIntercardinal()) {
            return new EightWayDirection[]{
                    EightWayDirection.byIndex(this.data2d, false),
                    EightWayDirection.byIndex(this.data2d + 1, false)
            };
        }
    ...
    public EightWayDirection[] getIntercardinalNeighbors() {
        if (this.isCardinal()) {
            return new EightWayDirection[]{
                    EightWayDirection.byIndex(this.data2d + 3, true),
                    EightWayDirection.byIndex(this.data2d, true)
            };
        }
